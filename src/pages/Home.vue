...
import {_getQuery, _nonEmpty} from "../util";
...
onMounted(() => {
  let query = _getQuery()
  if (_nonEmpty(query.cmd)) {
    _go('demo', query)
  }
})

const _go = (name, query) => {
  router.push({
    name: name,
    query: query
  })
...
<template>
  <div>
    <img :src="logoSvg" class="logo" alt="terminal-logo">
  ...
  <div class="btn-box">
    <button class="btn btn-local" @click="_go('demo', null)">Get Started →</button>
    <button class="btn btn-local" @click="_go('runner', null)">Code Online →</button>
  </div>
</template>
