...
<template>
  <div id="app">
    <div v-if="showLocal || showSsh" class="terminal-container">
      <LocalTerminal v-if="showLocal" :init-cmd="localInitCmd" @onClose="resetLocal" />

      <SshTerminal v-if="showSsh" @onClose="showSsh = false" />
    </div>
    <div v-else>
      <div>
        <img src="/logo.svg" class="logo" alt="terminal-logo">
        <h1 class="title">
          vue-web-terminal
        </h1>
        <p class="des">
          A lightweight, powerful and highly extensible web page emulation command line terminal plugin.
        </p>
        <p class="des">
          一个轻量、功能强大、拓展性高的网页仿真命令行终端插件。
        </p>
      ...
      <div class="btn-box">
        <button v-show="!showLocal && !showSsh" class="btn btn-local" @click="showLocal = true">
          Get Started →
        </button>
